...
# 정정: 진짜 Jev로 같은 프로브를 돌린 결과 (2026-09-21)

API 키를 받아 **TypeSafe의 실제 Jev(jev-1.13.0)** 로 위와 똑같은 프로브 3종을 돌렸다.
결론이 바뀐다: **"인코더 계열은 크기를 못 읽는다"는 것은 오픈 모조품들의 한계였지 Jev 자체의 한계가 아니었다.**

| 프로브 | 키워드 | jeff (GLiFormer 400M) | Laya (421M) | Qwen3-4B logprob | **진짜 Jev** |
|---|---|---|---|---|---|
| 캐릭터 이탈 Noul (14문장) | 6/14 | 6/14 | 6/14 | 11/14 | **14/14** |
| BMS 4지선다 — 숫자 JSON | — | 1/3 | 1/3 | — | **3/3** (신뢰도 80~100%) |
| BMS 4지선다 — 서술 문장 | — | 1/3 | 2/3 | — | **3/3** (98~100%) |
| 경제 스티어링 (크기·임계값) | — | 실패 | 실패 | 정확 | 대체로 정확 (아래) |

캐릭터 이탈 판정은 참 0.94~0.99 / 거짓 0.03~0.09으로 여유롭게 갈렸다. 함정 문장
("I cannot afford a new pickaxe…", "as an apprentice…")도 전부 통과했다 — 키워드 매처와
두 오픈 인코더가 모두 오탐한 자리다.

경제 스티어링은 4개 시나리오 중 3개를 규칙과 같게 판단했다(시약 부족→지갑 줍기, 금 넘침→예치).
숫자 JSON으로 주면 "시약 충분+금 넘침"에서 `buy_reagent`로 흔들렸다 — 서술 문장이 더 안정적이다.
상태 없이 후보 이름만 주면 confidence 0.30으로 정직하게 낮다.

**실측 비용·지연** (질문 2개짜리 호출 8회): 중앙값 **350 ms** (256~662), 호출당 약 414 입력 토큰,
**1000회에 약 $0.017**. 로컬 Qwen(150 ms, $0)보다 느리고 유료지만, 판단 품질은 확실히 위다.

jeff/Laya에 대한 위의 측정은 그대로 유효하다 — 그 모델들에 대한 기록으로 남긴다.

## 이 결론이 실제로 쓰인 곳

이 테스트베드의 측정은 [`hulryung-uo/anima3`](https://github.com/hulryung-uo/anima3) — 울티마 온라인을
사는 자율 에이전트 — 의 결정 계층 설계를 그대로 결정했다.

- jeff/Laya는 크기·임계값 판단에서 실패했으므로 **기본 백엔드가 되지 못했다**. 로컬 Qwen3-4B의
  logprob 모드가 기본이 되었고, jeff는 `--backend jeff`로 비교용으로만 남았다.
- Laya는 **화제 분류**에는 강했으므로(AI 발화 탐지 재현율 4/4, 41 ms) 들린 말을
  인사/질문/거래/위협으로 나누는 자리에 들어갔다.
- 진짜 Jev는 위 정정대로 두 프로브를 만점으로 통과했고, anima3에 `--backend jev`로 들어가
  PvP 결투 벤치마크에서 규칙·Qwen과 같은 조건으로 겨루고 있다.

즉 이 저장소의 프로브는 "어떤 모델이 좋은가"가 아니라 **"이 결정을 누구에게 맡길 것인가"**에
답하기 위한 것이었고, 답은 과제마다 달랐다.
